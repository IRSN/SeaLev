...
\noindent
Each row correspond to a given period $T$, e.g.
$T=100$ years, and gives the corresponding return level
$z(T)$ (column \verb@quant@) as well as confidence limits for
$z(T)$, here $95\,\%$ and
$90\,\%$.  It is possible to specify the wanted periods by using the
\verb@pred.period@ formal of \verb@Renouv@. The column \verb@prob@ gives the
probability of non-exceedance
$p(T)$ \textit{for an event}, not for an annual maxima. For instance, a
non-exceedance probability $p =
0.99$ corresponds to a level which on average is exceeded once in
$100$ \textit{events}, hence once in $100 / \lambda \approx \Sexpr{round(100 /
  lambda, digits = 1)}$ years.


Recall that $z(T) = q_Z(1 - 1 /T)$ with
$T = 1/[\widehat{\lambda} \times (1-p)]$, thus uncertainty on the estimation of
$\lambda$ has an impact on the estimation of $z(T)$. However, this uncertainty
is small for large periods.
